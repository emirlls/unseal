...
public class CapsuleLikeConfiguration : IEntityTypeConfiguration<CapsuleLike>
{
    public void Configure(EntityTypeBuilder<CapsuleLike> builder)
    {
        builder.ToTable(builder.GetTableName(),DatabaseConstants.SchemaName);
        builder.ConfigureByConvention();

        builder.HasOne(x=>x.Capsule)
            .WithMany(x=>x.CapsuleLikes)
